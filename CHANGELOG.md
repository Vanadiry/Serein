...
- 重构：抽取并复用部分流程的公共逻辑
- 前端抽取平台顺序常量与下载扩展名设置
- 下载器类型判定统一为单一来源，修正各入口判定不一致
- API 路由参数统一为 `{tracker_id}` + `PathValue`
- 移除冗余的日志初始化调用
- 补充单元测试
- 文档注明自定义下载器命令按空白切分、程序路径不支持空格

### Fixed

- 动态配置解析错误不再被静默忽略，改为记录告警
- 用户数据 `software.json` 改为原子写入
- 检查进度遮罩打开时，`/` 快捷键不再误触发跳转搜索
