...
每个规则表，为一个 `.toml` 文件，程序递归加载 `rules/` 下所有内容。  
...
[config]
type = "json"
url = "https://vanadiry.com/check{{token}}"
```

通常情况下，这个功能被用作填写 Token 等数据，但程序并未对此限制，可以随意定义。  
规则表的编写者应当对含有规则变量的规则，做出额外提示。
